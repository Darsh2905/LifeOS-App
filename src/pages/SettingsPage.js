...
import { Sun, Moon, Palette, Check, RotateCcw, AlertTriangle, X, LogOut, User, Mail, Shield, ImageIcon, Cloud, Brain, MapPin, CheckCircle2, XCircle, Link2, Unlink, Calendar as CalendarIcon } from 'lucide-react';
...
export default function SettingsPage() {
  ...
  const [calendar, setCalendar] = useState({ connected: false, configured: true, email: null, loading: true });
  const [calendarBusy, setCalendarBusy] = useState(false);

  const refreshCalendarStatus = async () => {
    try {
      const s = await api.get('/calendar/status');
      setCalendar({ ...s, loading: false });
    } catch {
      setCalendar(c => ({ ...c, loading: false }));
    }
  };
  ...
  // Re-fetch when a calendar connection event fires from App.js (post-OAuth redirect)
  useEffect(() => {
    const handler = () => refreshCalendarStatus();
  ...
  const disconnectGoogle = async () => {
    if (!window.confirm('Disconnect your Google Calendar from LifeOS?')) return;
    setCalendarBusy(true);
    try {
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        whileHover={{ y: -1, boxShadow: '0 8px 30px rgba(0,0,0,0.15)' }}
        className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-card)] p-5 relative overflow-hidden"
      >
        {/* Soft gradient accent behind the header */}
        <div
          className="pointer-events-none absolute -top-20 -right-20 h-56 w-56 rounded-full opacity-40"
          style={{ background: 'radial-gradient(closest-side, rgba(147,51,234,0.18), transparent 70%)' }}
        />

        <div className="flex items-center gap-3 mb-4 relative">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-purple-500/20 to-violet-500/10 ring-1 ring-purple-500/20 flex items-center justify-center">
            <Link2 size={18} className="text-purple-400" />
          </div>
          <div>
            <p className="text-sm font-medium text-[var(--color-text-primary)]">Integrations</p>
            <p className="text-xs text-[var(--color-text-muted)]">Connect external services to LifeOS</p>
          </div>
        </div>

        <div className="rounded-xl border border-[var(--color-border)] bg-white/[0.02] p-4 flex items-center gap-4 relative">
          ...
          <div className="flex-1 min-w-0">
              ...
              <AnimatePresence>
                {calendar.connected && (
                  <motion.span
                    ...
                    className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wider bg-emerald-500/10 text-emerald-400 ring-1 ring-emerald-500/20"
                  >
            ...
            <p className="text-xs text-[var(--color-text-muted)] truncate">
              {calendar.loading
                ? 'Checking status…'
                : calendar.connected
                  ? <>Syncing as <span className="text-[var(--color-text-secondary)]">{calendar.email}</span></>
                  : 'Sync your Google Calendar events and due-dated tasks.'}
            </p>
          </div>
          {calendar.connected ? (
            <motion.button
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-colors disabled:opacity-50"
            >
              <Unlink size={13} />
              Disconnect
            </motion.button>
            ...
            <motion.button
              whileHover={{ scale: 1.02, boxShadow: '0 0 24px rgba(147,51,234,0.2)' }}
              whileTap={{ scale: 0.98 }}
              onClick={connectGoogle}
              disabled={calendarBusy || !calendar.configured}
              className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold bg-gradient-to-r from-purple-500 to-violet-500 text-white shadow-lg shadow-purple-500/20 disabled:opacity-50 transition-all"
            >
              {calendarBusy ? (
                <motion.div
                  animate={{ rotate: 360 }}
