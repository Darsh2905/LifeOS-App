...
function startGoogleSignIn() {
  // In dev, the CRA dev server serves index.html for Accept: text/html navigations
  // even on /api paths, so we bypass the proxy by using the backend origin directly.
            ...
            {verifiedSuccess ? (
              ...
              <motion.div key="auth-form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <motion.button
                  ...
                  whileHover={{ y: -1, boxShadow: '0 10px 30px rgba(0,0,0,0.35)' }}
                  whileTap={{ scale: 0.99 }}
                  className="w-full mb-3 flex items-center justify-center gap-3 py-3 rounded-xl text-sm font-semibold text-[#1f1f1f] bg-white hover:bg-[#f7f7f7] transition-colors border border-black/5 shadow-[0_2px_8px_rgba(0,0,0,0.25)] relative overflow-hidden"
                >
                ...
                <div className="flex items-center gap-3 my-4 select-none">
                  <span className="flex-1 h-px bg-gradient-to-r from-transparent via-white/10 to-white/10" />
                  <span className="text-[10px] uppercase tracking-[0.25em] text-[#555]">or</span>
                  <span className="flex-1 h-px bg-gradient-to-l from-transparent via-white/10 to-white/10" />
                </div>
